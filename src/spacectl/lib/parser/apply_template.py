...
def get_value_by_dot(original_data, current_data, dotted_key: str):
# apply template을 통해 dot 표현에서 값을 가져옴.
    if '.' in dotted_key:
        ...
        return get_value_by_dot(original_data, value, rest)
    # dotted_key가 마지막 key일 때
        ...
        if value.startswith("${{"):
            ...
            if key == "tasks": # self에 대한 수정이 필요
                ...
                return get_value_by_dot(original_data, next_current_data, ".".join(["self", rest]))
            else:
                return get_value_by_dot(original_data, get_obj_value(current_data, key), ".".join(["self", rest]))
    ...
    for field in fields:
        ...
        if type(value) == str:
            if value.startswith("${{"):
                ...
                converted_value = get_value_by_dot(original_data, original_data, trimmed_value)
